...
try {
    ...
    // Get the student's name from the first result
...
// Set yellow color for table headers
$pdf->SetFillColor(255, 255, 0);

// Sub-header with class and faculty information
$pdf->SetFont('Arial', '', 8);
...
foreach ($grouped_results as $subject => $students) {
    ...
    $pdf->SetFillColor(255, 255, 0);// Set a light gray background for the subject name
    ...
    foreach ($students as $student) {
        ...
        // $pdf->Cell(40, 6, $student['class_name'], 1);
        $pdf->Cell(40, 6, $student['absent_date'], 1);
        $pdf->Cell(40, 6, $student['excuses'], 1);
        $pdf->Ln();
    }
    $pdf->Ln(5); // Add a little space after each subject group
}
$pdf->Ln(10);
$pdf->SetFont('Arial', 'I', 8);
$pdf->Cell(0, 10, 'Generated on: ' . date('Y-m-d'), 0, 0, 'R');
// Sanitize the student's name for use in the file name
